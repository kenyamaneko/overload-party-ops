...
# Cloud SQL の実処理は DB オーナーである overload-party-infra に workflow_dispatch で委譲する。
# 完了通知はワークフロー側から Slack に送られる。
...
async def _handle(
    response_url: str, text: str, *, command: str, action: str, action_jp: str,
) -> None:
    try:
        env = _parse_env(text)
        if env is None:
            await post_in_channel(
                response_url,
                f"環境を指定してください: `{command} dev` or `{command} stg`",
            )
            return
...
async def handle_start(response_url: str, text: str) -> None:
    ...
    await _handle(response_url, text, command="/db-start", action="up", action_jp="起動")


async def handle_stop(response_url: str, text: str) -> None:
    """Cloud SQL 停止ワークフローをディスパッチします。"""
    await _handle(response_url, text, command="/db-stop", action="down", action_jp="停止")
