...
from union_format import BANNER, render_source_header
...
def render_union(sources: list[dict]) -> str:
    ...
    for src in sources:
        chunks.extend(render_source_header(src["name"], src["repo"], src["ref"], src["path"]))
...
def render_seed_union(sources: list[dict]) -> str:
    ...
    for src in sources:
        for path, sql in src["seeds"]:
            chunks.extend(render_source_header(src["name"], src["repo"], src["ref"], path))
